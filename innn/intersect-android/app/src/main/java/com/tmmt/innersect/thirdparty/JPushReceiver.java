...
public class JPushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            ...
            if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
                ...
                hideNotification(context,notifactionId);

                String alert = bundle.getString(JPushInterface.EXTRA_ALERT);
                String extra = bundle.getString(JPushInterface.EXTRA_EXTRA);
                if (extra != null && !extra.isEmpty()) {
                    JSONObject json = new JSONObject(bundle.getString(JPushInterface.EXTRA_EXTRA));
                    //String schema = json.optString("extras");
                    String type= json.optString("type");
                    if(type.equals("moor")){
                        show(context,alert);
                    }
                    //String content= URLDecoder.decode(schema);
                    //KLog.d(content);
                }
                ...
                if (extra != null && !extra.isEmpty()) {
                    ...
                    String schema = json.optString("extras");
                    //TODO
                    Util.goToTarget(context, schema);
    ...
    private void show(Context context,String alert) {
        Activity activity = App.mRunning;
        if (activity != null) {

            View view=LayoutInflater.from(context).inflate(R.layout.popupwindow_custom,null);
            view.setOnClickListener(v -> Util.startMoor(context));
            ...
            mPopupWindow.setAnimationStyle(R.style.pop_down_anim);
            mPopupWindow.setBackgroundDrawable(new ColorDrawable(Color.WHITE));
            View rootView = activity.getWindow().getDecorView();
            mPopupWindow.showAtLocation(rootView, Gravity.TOP, 0, 0);
            rootView.postDelayed(()-> mPopupWindow.dismiss(),3000);
        }
    }
